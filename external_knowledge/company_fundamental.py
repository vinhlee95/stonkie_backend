...
def get_company_fundamental(ticker: str) -> dict | None:
    ...
    try:
        ...
        if not data or not data.get("Name") or not data.get("MarketCapitalization"):
            logger.error(
                "Invalid fundamental data found for ticker",
                extra={
                    "ticker": ticker,
                    "data": data,
                    "has_name": bool(data.get("Name")),
                    "has_market_cap": bool(data.get("MarketCapitalization")),
                },
            )
            return None
        ...
        print(ex)
        logger.error("Failed to fetch company fundamental data", extra={"ticker": ticker, "error": str(ex)})
