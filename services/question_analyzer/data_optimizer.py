...
class FinancialDataOptimizer:
    ...
    async def _fetch_annual_statements(
        ...
        if period_requirement.specific_years:
            ...
            logger.info(f"Fetched {len(statements_raw)} annual statements for years: {period_requirement.specific_years}")
    ...
    async def _fetch_quarterly_statements(
        ...
        if period_requirement.specific_quarters:
            ...
            logger.info(f"Fetched {len(statements_raw)} quarterly statements for: {period_requirement.specific_quarters}")
        elif period_requirement.num_periods:
            ...
            statements_raw = self.company_financial_connector.get_company_quarterly_financial_statements_recent(ticker, 4)
