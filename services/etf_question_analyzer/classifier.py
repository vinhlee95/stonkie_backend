...
class ETFQuestionClassifier:
    ...
    async def classify_question(self, ticker: str, question: str) -> tuple[ETFQuestionType, ETFDataRequirement]:
        """
        ...
        Returns:
            Tuple of (question_type, data_requirement)
        """
...
{{
  ...
  "reasoning": "brief explanation"
        ...
        try:
            ...
            return question_type, data_requirement

        except Exception as e:
            logger.error(f"Error classifying ETF question: {e}, response: {response_text[:200]}")
            # Fallback: if no ticker, assume general question
            if not has_ticker:
                return ETFQuestionType.GENERAL_ETF, ETFDataRequirement.NONE
            # Otherwise assume basic overview question
            return ETFQuestionType.ETF_OVERVIEW, ETFDataRequirement.BASIC
