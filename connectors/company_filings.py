    ...
    def _get_filings_from_database(self, ticker: str, period: str) -> List[FilingDto]:
        ...
        try:
            if period == "annual":
                ...
                records = (
                    session.query(CompanyFinancialStatement)
                    .filter(
                ...
                for record in records:
                    filings.append(FilingDto(url=record.filing_10k_url, period_end_year=record.period_end_year))
                ...
                records = (
                    session.query(CompanyQuarterlyFinancialStatement)
                ...
                for record in records:
                    ...
                    try:
                        ...
                        if " " in quarter_str:
                            int(quarter_str.split(" ")[-1])
                        elif "-" in quarter_str:
                            int(quarter_str.split("-")[0])
                        else:
            ...
            for filing in sec_filings:
                try:
                    ...
                    if time_str:
                        ...
                        url = filing.get("URL", "")
                        if url:
                            filings.append(FilingDto(url=url, period_end_year=year))
