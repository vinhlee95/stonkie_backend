...
class _SECFilingsFetcher:
    ...
    def filter_filings(
        self, filings_data: Dict, filing_type: Optional[str] = None, limit: Optional[int] = None
    ) -> List[Dict]:
        """
    ...
    def get_accessible_filing_urls(self, accession_number: str, primary_document: str, cik: str) -> Dict[str, str]:
        ...
        urls["interactive_viewer"] = (
            f"https://www.sec.gov/cgi-bin/viewer?action=view&cik={cik}&accession_number={accession_number.replace('-', '')}"
        )

        # 6. XBRL files (if available) - for structured data
        urls["xbrl_instance"] = (
            f"{edgar_base}/{primary_document.replace('.htm', '_htm.xml')}"
    ...
    def fetch_filings_for_ticker(
        self, ticker: str, filing_type: Optional[str] = None, limit: int = 10
    ) -> Optional[List[Dict]]:
        """
        ...
        for filing in filings:
            filing["accessibleUrls"] = self.get_accessible_filing_urls(
                filing["accessionNumber"], filing["primaryDocument"], cik
            )
            ...
            filing["documentUrl"] = (
                filing["htmUrls"][0]
                if filing["htmUrls"]
                else (filing["readableUrls"][0] if filing["readableUrls"] else "")
            )
