...
from connectors.database import engine, Base
...
def analyze_records_comprehensive(records: List[CompanyFinancialStatement]) -> Tuple[Dict, Dict]:
    ...
    initial_stats = {
        ...
        'already_filled': 0,
        'need_backfill': 0,
        'company_details': defaultdict(lambda: {
            'total': 0,
            'already_filled': 0,
            'need_backfill': 0,
            'years': []
        }),
        'year_coverage': defaultdict(lambda: {
            'total': 0,
            'filled': 0,
            'missing': 0
        })
    ...
    for record in records:
        ...
        companies_data[ticker].append({
            'id': record.id,
            'year': year,
            'current_url': record.filing_10k_url,
            'has_url': has_url
        })
...
def main():
    ...
    summary_data['failed_to_fill'] = summary_data['total_records'] - summary_data['already_filled'] - summary_data['newly_filled']
